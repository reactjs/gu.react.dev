...
## શરતી રીતે JSX રીટર્ન કરવું {/_conditionally-returning-jsx_/}
...
### શરતી રીતે `null` સાથે કશું જ રીટર્ન કરશે નહીં.{/_conditionally-returning-nothing-with-null_/}
...
## શરતી રીતે JSX સમાવિષ્ટ કરવું {/_conditionally-including-jsx_/}
...
### શરતી (ટર્નરી) ઓપરેટર (`? :`) {/_conditional-ternary-operator--_/}
...
તમે આ રીતે વાંચી શકો છો: _"જો `isPacked` true છે, તો (`?`) `name + ' ✅'` રેન્ડર કરો, અન્યથા (`:`) `name` રેન્ડર કરો"_.

<DeepDive>

#### શું આ બે ઉદાહરણ સંપૂર્ણ રીતે સમાન છે? {/_are-these-two-examples-fully-equivalent_/}
...
### લોજિકલ AND ઑપરેટર (`&&`) {/_logical-and-operator-_/}
...
તમે આને આ રીતે વાંચી શકો છો _"જો `isPacked` હોય, તો (`&&`) ચેકમાર્ક રેન્ડર કરો, અન્યથા કશું જ રેન્ડર ન કરો"_.
...
### JSX ને શરતી રીતે વેરીએબલને ફાળવવું{/_conditionally-assigning-jsx-to-a-variable_/}
...
#### અપૂર્ણ items માટે આઇકન બતાવવા `? :` નો ઉપયોગ કરો. {/_show-an-icon-for-incomplete-items-with--_/}
...
#### આઇટમનું મહત્વ દર્શાવવા માટે `&&` નો ઉપયોગ કરો. {/_show-the-item-importance-with-_/}
...
#### `? :` ની શ્રેણી ને `if` અને વેરીએબલમાં પુનર્ગઠિત કરો. {/_refactor-a-series-of---to-if-and-variables_/}
